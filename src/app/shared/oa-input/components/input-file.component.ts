...
export class InputFileComponent {
    ...
    @Input() public showTitleDescription = false;
        ...
        for (let index = 0; index < files.length; index++) {
            ...
            if (this.model.length < limit) {
                ...
                if (this.isTypeEnabled(files[index])) {
                    ...
                    this.setTitle(file, files[index]);
                    // Checks the icon
     ...
     * @param file
     */
    public isTypeImage(file: any): boolean {
  ...
  private setTitle(file: File, inputFile: any): void {
    file.title = inputFile.name;
  }
